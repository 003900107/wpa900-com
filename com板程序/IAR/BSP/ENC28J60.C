...
void enc28j60ReadBuffer(unsigned int len, unsigned char* data)
{
	...
	while(len)
        ...
        len--;
        // read data
        *data = (unsigned char)SPI3_ReadWrite(0);
        data++;
		}
	*data='\0';
	ENC28J60_CSH();
...
void enc28j60Init(unsigned char* macaddr)
{
	...
	enc28j60PhyWrite(PHLCON,0x0746);  //0x0476
    
	enc28j60clkout(2); // change clkout from 6.25MHz to 12.5MHz
    
    enc28j60_sem_lock = OSSemCreate(1); //添加
        
        /*测试*/
    //unsigned char version;
    //version = enc28j60getrev();
    //version = enc28j60Read(MAADR0);
    //printf("version = %d\n",version);
...
int8_t enc28j60PacketSend(struct pbuf* p)
{
  INT8U err;
  struct pbuf* q;
  ...
  OSSemPend(enc28j60_sem_lock,0,&err);
	...
	enc28j60Write(ETXNDL, (TXSTART_INIT+p->tot_len)&0xFF);
	enc28j60Write(ETXNDH, (TXSTART_INIT+p->tot_len)>>8);
...
struct pbuf *enc28j60PacketReceive(void)
{
  INT8U err;
  struct pbuf* p;
