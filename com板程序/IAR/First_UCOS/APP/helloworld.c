...
//测试
#define IP_ADDR_1_VALUE 0xc0a80101UL   //0x00000000UL
const struct ip_addr ip_addr_1 = { IP_ADDR_1_VALUE };
#define IP_ADDR_1         ((struct ip_addr *)&ip_addr_1)
...
{     
  ...
  tcp_recv(pcb, HelloWorld_recv);      /*以太网测试时暂时屏蔽函数*/
  //tcp_recv(pcb, DataTest_recv);          /*以太网测试时调用函数*/
...
void HelloWorld_init(void)
{
  ...
  err = tcp_bind(pcb, IP_ADDR_ANY, 23);       //IP_ADDR_ANY  IP_ADDR_1
  //printf("\nTCP绑定结果：%d",err);
